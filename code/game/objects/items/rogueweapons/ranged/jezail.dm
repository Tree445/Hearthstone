...
/obj/item/gun/ballistic/revolver/grenadelauncher/jezail
	name = "jezail"
...
/obj/item/gun/ballistic/revolver/grenadelauncher/jezail/getonmobprop(tag)
	. = ..()
	if(tag)
		switch(tag)
			if("gen")
...
/datum/intent/shoot/jezail
	chargedrain = 0 //no drain to aim a crossbow

/datum/intent/shoot/jezail/get_chargetime()
...
/datum/intent/arc/jezail
	chargetime = 1
	chargedrain = 0 //no drain to aim a crossbow

/datum/intent/arc/jezail/get_chargetime()
	if(mastermob && chargetime)
		var/newtime = chargetime
		//skill block
		newtime = newtime + 18
...
/obj/item/gun/ballistic/revolver/grenadelauncher/jezail/shoot_with_empty_chamber()
	if(cocked)
		playsound(src.loc, 'sound/foley/musketcock.ogg', 100, FALSE)
		cocked = FALSE
		update_icon()

/obj/item/gun/ballistic/revolver/grenadelauncher/jezail/attack_self(mob/living/user)
	if(chambered)
		..()
	else
		if(!cocked)
			to_chat(user, span_info("I ready the jezail to be fired..."))
			if(do_after(user, 100 - user.STASTR, target = user))
				playsound(user, 'sound/foley/musketcock.ogg', 100, FALSE)
				cocked = TRUE
		else
			to_chat(user, span_warning("I carefully de-cock the jezail."))
			cocked = FALSE
	update_icon()

/obj/item/gun/ballistic/revolver/grenadelauncher/jezail/attackby(obj/item/A, mob/user, params)
	if(istype(A, /obj/item/ammo_box) || istype(A, /obj/item/ammo_casing))
		if(cocked)
			...
			to_chat(user, span_warning("I need to cock the jezail first!"))


/obj/item/gun/ballistic/revolver/grenadelauncher/jezail/process_fire/(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	if(user.client)

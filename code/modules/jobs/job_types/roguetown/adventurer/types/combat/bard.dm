...
/datum/outfit/job/roguetown/adventurer/bard/pre_equip(mob/living/carbon/human/H)
	...
	var/classes = list("Bard","Arcanist",)
	...
	switch(classchoice)
		...
		if("Arcanist")
			...
			H.mind.adjust_skillrank(/datum/skill/misc/reading, 3, TRUE)
			H.mind.adjust_skillrank(/datum/skill/combat/swords, 3, TRUE) // one expert level skill if you take college of swords
			H.mind.adjust_skillrank(/datum/skill/combat/wrestling, 2, TRUE)
			...
			H.change_stat("endurance", 1)
			H.change_stat("speed", -2) // you get arcane, use it, I'm not allowing battle mages		
